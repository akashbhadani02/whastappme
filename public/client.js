...
(() => {
  ...
  const peers=new Map();
  // One composite recorder per participant. The recorded video is a live canvas
  // containing EVERY visible call feed (local + all remote feeds), not one file per feed.
  let compositeRecorder=null, compositeChunks=[], compositeStartedAt=0, compositeCanvas=null, compositeCtx=null, compositeTimer=null, compositeAudioContext=null, compositeAudioDestination=null;
  const remoteStreams=new Map();
  const recordingMime=()=>['video/webm;codecs=vp9,opus','video/webm;codecs=vp8,opus','video/webm'].find(x=>window.MediaRecorder?.isTypeSupported?.(x))||'';

  function feedEntries(){
    const out=[];
    if(localStream) out.push({id:'local-'+socket.id,name:name||'You',stream:localStream});
    remoteStreams.forEach((stream,id)=>{ const x=peers.get(id); out.push({id,name:x?.name||'Participant',stream}); });
    return out;
  }
  function drawComposite(){
    if(!compositeCtx||!compositeCanvas)return;
    const feeds=feedEntries();
    const cols=Math.max(1,Math.ceil(Math.sqrt(Math.max(1,feeds.length))));
    const rows=Math.max(1,Math.ceil(Math.max(1,feeds.length)/cols));
    const w=640,h=360; compositeCanvas.width=cols*w; compositeCanvas.height=rows*h;
    compositeCtx.fillStyle='#111'; compositeCtx.fillRect(0,0,compositeCanvas.width,compositeCanvas.height);
    feeds.forEach((f,i)=>{
      const x=(i%cols)*w,y=Math.floor(i/cols)*h;
      const video=document.querySelector(`.call-tile[data-peer="${CSS.escape(f.id)}"] video`) || (f.id==='local-'+socket.id?localCallVideo:null);
      const track=f.stream?.getVideoTracks?.()[0];
      if(video && video.readyState>=2 && track?.enabled && video.videoWidth){
        const vw=video.videoWidth,vh=video.videoHeight,scale=Math.max(w/vw,h/vh),dw=vw*scale,dh=vh*scale;
        compositeCtx.save(); compositeCtx.beginPath(); compositeCtx.rect(x,y,w,h); compositeCtx.clip();
        compositeCtx.drawImage(video,x+(w-dw)/2,y+(h-dh)/2,dw,dh); compositeCtx.restore();
      } else {
        compositeCtx.fillStyle='#263238'; compositeCtx.fillRect(x,y,w,h);
        compositeCtx.fillStyle='#fff'; compositeCtx.font='bold 24px sans-serif'; compositeCtx.textAlign='center';
        compositeCtx.fillText(f.name||'Participant',x+w/2,y+h/2);
      }
      compositeCtx.fillStyle='rgba(0,0,0,.55)'; compositeCtx.fillRect(x+10,y+h-42,Math.min(w-20,260),30);
      compositeCtx.fillStyle='#fff'; compositeCtx.font='16px sans-serif'; compositeCtx.textAlign='left'; compositeCtx.fillText(f.name||'Participant',x+20,y+h-21);
    });
  }
  function addAudioStream(stream){
    if(!compositeAudioContext||!compositeAudioDestination||!stream)return;
    try{ const tracks=stream.getAudioTracks?.(); if(tracks?.length){ const source=compositeAudioContext.createMediaStreamSource(new MediaStream(tracks)); source.connect(compositeAudioDestination); } }catch(_){}
  }
  function startCompositeRecording(){
    if(compositeRecorder||!window.MediaRecorder||!activeCallId)return;
    const mime=recordingMime(); if(!mime)return;
    try{
      compositeCanvas=document.createElement('canvas'); compositeCanvas.width=640; compositeCanvas.height=360; compositeCtx=compositeCanvas.getContext('2d');
      const videoStream=compositeCanvas.captureStream(20);
      compositeAudioContext=new (window.AudioContext||window.webkitAudioContext)();
      compositeAudioDestination=compositeAudioContext.createMediaStreamDestination();
      addAudioStream(localStream); remoteStreams.forEach(addAudioStream);
      const mixed=new MediaStream([...videoStream.getVideoTracks(),...compositeAudioDestination.stream.getAudioTracks()]);
      compositeStartedAt=Date.now(); compositeChunks=[];
      compositeRecorder=new MediaRecorder(mixed,{mimeType:mime});
      compositeRecorder.ondataavailable=e=>{if(e.data?.size)compositeChunks.push(e.data)};
      compositeRecorder.onstop=async()=>{
        const stoppedAt=Date.now(), durationMs=Math.max(0,stoppedAt-compositeStartedAt);
        const chunks=compositeChunks.splice(0); const blob=chunks.length?new Blob(chunks,{type:mime}):null;
        try{ if(blob?.size){ await fetch('/api/call-recordings/upload',{method:'POST',headers:{'Content-Type':'application/octet-stream','X-Call-Id':activeCallId,'X-Group-Id':currentGroupId,'X-Feed-Id':'composite-'+socket.id,'X-Feed-Name':name||'You','X-User-Id':String(userId||''),'X-Mime-Type':mime,'X-Recording-Start':new Date(compositeStartedAt).toISOString(),'X-Recording-End':new Date(stoppedAt).toISOString(),'X-Recording-Duration-Ms':String(durationMs)},body:blob}); } }catch(_){}
        try{compositeAudioContext?.close()}catch(_){} compositeAudioContext=null; compositeAudioDestination=null; compositeCanvas=null; compositeCtx=null; compositeRecorder=null;
      };
      compositeRecorder.start(500);
      compositeTimer=setInterval(drawComposite,50);
    }catch(_){ compositeRecorder=null; }
  }
  function stopCompositeRecording(){
    if(compositeTimer){clearInterval(compositeTimer);compositeTimer=null;}
    try{if(compositeRecorder&&compositeRecorder.state!=='inactive')compositeRecorder.stop();}catch(_){}
  }
  ...
  function createPeer(id,nm,offer){let x=peers.get(id);if(x)return x.pc;const pc=new RTCPeerConnection(RTC_CONFIG);x={pc,name:safeText(nm||'Participant',60)};peers.set(id,x);if(localStream)localStream.getTracks().forEach(t=>pc.addTrack(t,localStream));
    ...
    pc.ontrack=e=>{const st=e.streams?.[0]||new MediaStream([e.track]); remoteStreams.set(id,st); tile(id,x.name,st); if(e.track.kind==='audio') addAudioStream(st);};
  ...
  function end(notify=true){const id=activeCallId;if(notify&&id)socket.emit('call-leave',{callId:id,name,userId});[...peers.keys()].forEach(removePeer);stopCompositeRecording(); if(localStream){localStream.getTracks().forEach(t=>t.stop());localStream=null}if(localCallVideo)localCallVideo.srcObject=null;activeCallId='';activeCallType='';callStartedByMe=false;callModal.classList.add('hidden');callStage.querySelectorAll('.call-tile').forEach(x=>x.remove());callEmpty.style.display='flex';recordingNoticeShown=false;updateStatus()}
