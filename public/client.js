...
const sessionGroupPasswords = new Map();
let joinedSocketGroupId = '';
...
async function verifyAndOpenGroup() {
  ...
  try {
    ...
    sessionGroupPasswords.set(String(group.id), password);
...
async function joinGroup(groupId, openAfter=true) {
  ...
  joinedSocketGroupId = String(currentGroupId);
  await syncMessages();
...
socket.on('connect', async () => {
  ...
  const gid = String(currentGroupId || 'main');
  if (gid !== 'main' && sessionGroupPasswords.has(gid)) {
    const password = sessionGroupPasswords.get(gid);
    socket.emit('authorize-group', { groupId: gid, password }, auth => {
      if (!auth?.ok) { joinedSocketGroupId = ''; return; }
      socket.emit('join-group', { groupId: gid }, response => {
        if (response?.ok) {
          joinedSocketGroupId = gid;
          syncMessages().finally(markVisibleMessagesRead);
        } else { joinedSocketGroupId = ''; }
      });
    });
  } else if (gid === 'main') {
    socket.emit('join-group', { groupId: 'main' }, response => {
      if (response?.ok) joinedSocketGroupId = 'main';
      syncMessages().finally(markVisibleMessagesRead);
    });
  }
});
...
(() => {
  ...
  function createPeer(id,nm,offer){let x=peers.get(id);if(x)return x.pc;const pc=new RTCPeerConnection(RTC_CONFIG);x={pc,name:safeText(nm||'Participant',60),pendingIce:[]};peers.set(id,x);if(localStream)localStream.getTracks().forEach(t=>pc.addTrack(t,localStream));
    pc.onicecandidate=e=>{if(e.candidate&&activeCallId)socket.emit('call-signal',{callId:activeCallId,to:id,kind:'ice',data:e.candidate})};
  ...
  socket.on('call-signal',async d=>{
    ...
    if(d.kind==='offer'){
      const pc=createPeer(d.from,'Participant',false); x=peers.get(d.from);
      try{
        await pc.setRemoteDescription(new RTCSessionDescription(d.data));
        if(x?.pendingIce?.length){for(const c of x.pendingIce){try{await pc.addIceCandidate(new RTCIceCandidate(c))}catch(_){}}x.pendingIce=[];}
      ...
      if(x)try{await x.pc.setRemoteDescription(new RTCSessionDescription(d.data));if(x.pendingIce?.length){for(const c of x.pendingIce){try{await x.pc.addIceCandidate(new RTCIceCandidate(c))}catch(_){}}x.pendingIce=[];}}catch(_){}
    }else if(d.kind==='ice'){
      if(!x){createPeer(d.from,'Participant',false);x=peers.get(d.from);}
      if(x){
        if(x.pc.remoteDescription?.type)try{await x.pc.addIceCandidate(new RTCIceCandidate(d.data))}catch(_){}
        else x.pendingIce.push(d.data);
      }
    }
  });
