...
function callStageAddParticipant(id, label, stream=null, muted=false, hasVideo=false){
  ensureCallMediaUI();
...
async function createPeer(remoteId, remoteName, initiator){
  ...
  pc.ontrack=e=>{ const stream=e.streams?.[0]; if(!stream) return; if(e.track?.kind==='video') callStageAddVideo(remoteId,stream,remoteName,false); else callStageAddAudio(remoteId,stream,remoteName); };
