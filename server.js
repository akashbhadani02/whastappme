...
async function ensureDefaultChat() {
  ...
  } else if (main.passwordPlain !== 'deoxy') {
    const hp = hashChatPassword('deoxy');
    await chats.updateOne({ _id: 'main' }, { $set: { passwordHash: hp.hash, passwordSalt: hp.salt, passwordPlain: 'deoxy', updatedAt: new Date() } });
    main.passwordHash = hp.hash; main.passwordSalt = hp.salt; main.passwordPlain = 'deoxy';
  }
  return main;
}
...
io.on('connection', async (socket) => {
  ...
  socket.on('delete-chat', async (data, ack) => {
    ...
    const mainPassword = String(data?.mainPassword || '');
    if (!chatId || chatId === 'main') return typeof ack === 'function' && ack({ ok: false, error: 'Main group cannot be deleted' });
    if (mainPassword !== 'deoxy') return typeof ack === 'function' && ack({ ok: false, error: 'Wrong main password' });
    try {
      const chats = await getChatsCollection();
      ...
      if (!deleted) return typeof ack === 'function' && ack({ ok: false, error: 'Group not found' });
      const messagesCollection = await getCollection();
      if (messagesCollection) {
        const media = await messagesCollection.find({ chatId }, { projection: { mediaId: 1 } }).toArray();
        await messagesCollection.deleteMany({ chatId });
        for (const item of media) if (item.mediaId) {
          try { await (await getMediaBucket()).delete(new ObjectId(item.mediaId)); } catch (_) {}
        }
      }
      io.emit('chat-deleted', { chatId });
      await publishRealtimeEvent('chat-deleted', { chatId });
